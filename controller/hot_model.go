...
func SetHotModel(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&request); err != nil || request.IsHot == nil || strings.TrimSpace(request.ModelName) == "" || utf8.RuneCountInString(request.ModelName) > 255 {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "model_name and boolean is_hot are required"})
		return
	}
	if err := model.SetHotModel(request.ModelName, *request.IsHot); err != nil {
