...
- `PUT /api/hot-models/` accepts `{ "model_name": "vendor/model", "is_hot": true, "created_time": 1700000000 }`. `created_time` is optional, uses Unix seconds, and may be `0` to mark the time unknown. Explicit `false` removes the hot flag. Unavailable models and invalid payloads are rejected.
...
Administrators can edit the catalog entry time directly in the management table. This is intended for backfilling historical models whose initial time is unknown; changing the hot flag alone does not change the entry time.
